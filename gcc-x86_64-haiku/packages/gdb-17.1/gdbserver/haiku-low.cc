...
haiku_process_target::resume (struct thread_resume *resume_info, size_t n)
{
  for (size_t i = 0; i < n; ++i)
    {
      if (resume_info[i].thread.tid_p ())
        {
          thread_info *info = find_thread_ptid (ptid_t (
              resume_info[i].thread.pid (), 0, resume_info[i].thread.tid ()));
          if (info != nullptr)
            regcache_invalidate_thread (info);
        }
      else if (resume_info[i].thread.pid () > 0)
        regcache_invalidate_pid (resume_info[i].thread.pid ());
      else
        regcache_invalidate ();

      /* TODO: What does the step_range_[start/end] mean?  */
      if (haiku_nat::resume (resume_info[i].thread, resume_info[i].kind,
                             resume_info[i].sig)
          < 0)
        {
